...
async def hfmm(_, message):
    ...
    if status in ["ON", "on", "On"]:
        ...
        if message.chat.id not in DISABLED_GROUPS:
            await lel.edit(
                f" - البوت مطفي ترا لتلح **{message.chat.title}**"
            )
            return
        ...
        await lel.edit(
            f" - تم اطفاء البوت بنجاح**{message.chat.title}**"
        )
    else:
        await message.reply_text("لتشغيل البوت اكتب الموسيقى on لاطفاء البوت اكتب الموسيقى off يرجى ملاحظة ان الامر للمطورين فقط")
...
async def play(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            " انت  مستخدم مجهول ** -ارجع ادمن- **!\n│\n╰ وراها تكدر تشغل البوت."
        )

    if message.chat.id in DISABLED_GROUPS:
        await message.reply(
            " **- المطور ضايج وطفة البوت راسلة وكله شبيك رحمه لدينك! **"
        )
        return
    ...
    try:
        ...
        if message.chat.username:
            try:
                ...
                await message.reply_text(
                    f" **- فشل حساب المساعد في الانضمام والسب **\n\n**Reason**:{e}"
                )
                return
        else:
            try:
                ...
                return await message.reply_text(
                    f" **- فشل حساب المساعد في الانضمام والسبب**\n\n**Reason**:{e}"
                )
    ...
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"❌ **مقاطع الفيديو أطول من **{DURATION_LIMIT} **الدقائق غير مسموح بها**!"
            )

        file_name = get_file_name(audio)
        url = f"https://t.me/{UPDATE}"
        title = "Wa"
        thumb_name = "https://graph.org/file/25a5e3c6aed24fb5adc55.jpg"
        ...
        try:
            ...
            title = "NaN"
            thumb_name = "https://graph.org/file/9d9462705727807a189e5.jpg"
            duration = "NaN"
            views = "NaN"
            keyboard = InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="YouTube 🎬", url="https://youtube.com")]]
    ...
    if await is_active_chat(message.chat.id):
        ...
        await message.reply_photo(
            photo="https://graph.org/file/16ad8802968697be7d4e7.jpg",
    ...
    os.remove("https://graph.org/file/16ad8802968697be7d4e7.jpg")
